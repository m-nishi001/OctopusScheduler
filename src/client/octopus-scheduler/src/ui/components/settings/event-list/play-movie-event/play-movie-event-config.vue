<template>
    <div class="event-config">
        <h3>{{ isNew ? '動画再生イベント新規作成' : '動画再生イベント編集' }}</h3>
        ...
        <div class="config-item">
            <label>動画アセット:</label>
            <select v-model="localConfig.movieId" class="admin-input">
...
import { PlayMovieEventDetail } from '../../../../../model/domains/schedule-event/entity/events/play-movie-event';
...
const converter = container.resolve(PlayMovieEventConverter);
...
const loadConfig = () => {
    if (props.isNew) {
        ...
        localConfig.value.movieId = detail.movieId || "";
    }
};
...
const handleSaveClick = async () => {
    ...
    await handleSave(async () => {
        const detail = new PlayMovieEventDetail(localConfig.value.movieId);
        const eventData = {
