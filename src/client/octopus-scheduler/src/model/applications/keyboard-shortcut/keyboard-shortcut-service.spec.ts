...
class InMemoryRepository extends KeyboardShortcutRepository {
  ...
  async saveKeyboardShortcuts(shortcuts: any[]) {
    this.storage.shortcuts = shortcuts.map((s) => s.serialize ? s.serialize() : s);
  }
  async getConfig(){ return {enabled: true}; }
  async saveConfig(){ return; }
  async syncWithServer(){ return; }
}
...
describe('KeyboardShortcutService', () => {
  ...
  beforeEach(() => {
    ...
    container.register(IKeyboardShortcutRepositoryToken, { useClass: InMemoryRepository });
    // register a dummy schedule event converter to avoid DI error
    container.register(IScheduleEventConverterToken, {
      useValue: {
        getType: () => 'Dummy',
        revive: (data: any) => ({ id: data.id, type: data.type, serialize: () => [] }),
      },
    });
  ...
  it('matches 3-key sequence', async () => {
    ...
    const now = new Date();
    const event = PlayAudioEvent.fromParams({ id: 'e1', startTime: now, endTime: new Date(now.getTime() + 1000), audioId: 'a1', fadeOutDuration: 0, processedAt: null, registeredAt: now, updatedAt: now });
    const shortcut = new KeyboardShortcut({ id: 's1', keys: ['Control', 's', '1'], action: event });
  ...
  it('detects longer prefix', async () => {
    const now = new Date();
    const eventA = PlayAudioEvent.fromParams({ id: 'e1', startTime: now, endTime: new Date(now.getTime() + 1000), audioId: 'a1', fadeOutDuration: 0, processedAt: null, registeredAt: now, updatedAt: now });
    const eventB = PlayAudioEvent.fromParams({ id: 'e2', startTime: now, endTime: new Date(now.getTime() + 1000), audioId: 'a2', fadeOutDuration: 0, processedAt: null, registeredAt: now, updatedAt: now });
    const shortCut = new KeyboardShortcut({ id: 's1', keys: ['Control', 's'], action: eventA });
    const longCut = new KeyboardShortcut({ id: 's2', keys: ['Control', 's', '1'], action: eventB });
    await service.saveKeyboardShortcuts([shortCut, longCut]);

    const hasLonger = await service.hasLongerShortcutWithPrefix(['Control', 's']);
