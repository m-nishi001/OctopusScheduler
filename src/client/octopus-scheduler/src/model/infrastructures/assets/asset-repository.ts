...
export class AssetRepository implements IAssetRepository {
    ...
    constructor() {
        ...
        this.assetStorage = new LocalStorageService(StorageConfig.getDbName(), "AssetData");
    }
    ...
    async updateName(assetId: string, newName: string): Promise<void> {
        try {
            ...
            await this.service
                ...
                .withFailuered(message => {
                    ...
                    throw new Error("Failed to update asset on remote.");
                })
                .invoke();

            // On success, remove local data
            const asset = (await this.findById(assetId))!;
            asset.updateAssetName(newName);

            await this.assetStorage.save(assetId, asset.serialize());

            console.log(`Asset with ID ${assetId} Name ${newName} updated successfully (remote + local).`);

        } catch (error) {
            console.error(`Failed to update asset with ID ${assetId}:`, error);
    ...
    async delete(assetId: string): Promise<void> {
        try {
            ...
            console.log(`Asset with ID ${assetId} deleted successfully (remote + local).`);
    ...
    async sync(): Promise<void> {
        try {
            ...
            const deletingIds = Array.from(localAssets.values())
                .filter(localAsset => !remoteMetadatas.some(remoteMetadata => remoteMetadata.assetId === localAsset.assetId))
                .map(localAsset => localAsset.assetId);
            if (deletingIds.length > 0) await this.assetStorage.removeMultiple(deletingIds);
            ...
            if (needsFetchIds.length > 0) {
                ...
                const assetsToSave = new Map(fetchedAssets.map(asset => [asset.assetId, asset.serialize()]));
                await this.assetStorage.saveMultiple<Asset>(assetsToSave);
            }

            console.log(`Sync completed. Deleted ${deletingIds.length} assets, fetched ${needsFetchIds.length} new assets.`);
    ...
    private async getRemoteMetadatas(): Promise<{ assetId: string; updatedAt: Date }[]> {
    ...
    private async fetchAssetDatas(assetIds: string[]): Promise<Asset[]> {
        ...
        const promises = assetIds.map(assetId =>
            this.service
                ...
                .withFailuered(message => console.error("Failed to fetch asset data:", message)));
