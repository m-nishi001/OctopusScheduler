...
export function usePrizeSync(prizeRepoArg?: IPrizeRepository, assetDataServiceArg?: AssetDataService, prizeServiceArg?: PrizeService) {
  ...
  const confirmPrizesSyncMode = async (mode: 'drive' | 'local', onFetch?: () => Promise<void>) => {
    ...
    syncMessage.value = '';
    try {
      await assetDataService.syncAssetData((msg: string) => { syncMessage.value = msg; });
  ...
  const performReplaceFromDrive = async (fetchAssets: () => Promise<void>, fetchPrizes?: () => Promise<void>) => {
    ...
    try {
      ...
      if (updatedPrizes.length) console.log('performReplaceFromDrive updated prizes', updatedPrizes.map(p => p.id));
  ...
  return { syncing, syncMessage, showSyncModeModal, showReplaceWarningModal, confirmPrizesSyncMode, performReplaceFromDrive };
}
