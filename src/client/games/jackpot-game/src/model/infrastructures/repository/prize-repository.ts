...
export class PrizeRepository {
  ...
  async fetchPrizes(): Promise<Prize[]> {
    ...
    return new Promise((resolve, reject) => {
      this.gasService
        .createCall<{ prizes: Prize[] }>('JackpotApiService.getPrizes')
  ...
  async savePrize(prize: Prize): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.gasService
        .createCall<void>('JackpotApiService.savePrize', prize)
  ...
  prizes = prizes.map((p: Prize) => p.id === prize.id ? prize : p);
    ...
    return new Promise((resolve, reject) => {
      this.gasService
        .createCall<void>('JackpotApiService.updatePrize', prize)
        .withSuccessed(() => resolve())
        .withFailuered((msg: string) => reject(new Error(msg)))
        .invoke();
    });
  ...
  prizes = prizes.filter((p: Prize) => p.id !== prizeId);
    ...
    return new Promise((resolve, reject) => {
      this.gasService
        .createCall<void>('JackpotApiService.deletePrize', { prizeId })
        .withSuccessed(() => resolve())
        .withFailuered((msg: string) => reject(new Error(msg)))
        .invoke();
    });
