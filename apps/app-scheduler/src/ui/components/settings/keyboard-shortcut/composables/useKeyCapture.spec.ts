...
describe("useKeyCapture", () => {
  ...
  it("captures 3-key sequence across keydown events", async () => {
    ...
    window.dispatchEvent(
      new KeyboardEvent("keydown", { key: "Control", ctrlKey: true } as any)
    );
    // then press S with control held
    window.dispatchEvent(
      new KeyboardEvent("keydown", { key: "s", ctrlKey: true } as any)
    );
    // then press 1
    window.dispatchEvent(new KeyboardEvent("keydown", { key: "1" } as any));
    expect(capturedKeys.value).toEqual(
      ["Control", "s", "1"] /* should be trimmed to 3 */
    );
