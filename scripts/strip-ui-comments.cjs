...
const uiDir = path.join(__dirname, '..', 'src', 'client', 'games', 'jackpot-game', 'src', 'ui');
...
function preserveComment(content) {
  ...
  return /@license|@preserve|@ts-ignore|eslint-disable|eslint-enable|prettier|istanbul|global|exported|@ts-ignore|@ts-expect-error/.test(content);
}

function stripVueComments(content) {
  // Split into template, script, style parts
  return content.replace(/<template([\s\S]*?)>([\s\S]*?)<\/template>/i, (m, attrs, inner) => {
    ...
    const s = inner.replace(/\/\*([\s\S]*?)\*\//g, (cm) => preserveComment(cm) ? cm : '');
