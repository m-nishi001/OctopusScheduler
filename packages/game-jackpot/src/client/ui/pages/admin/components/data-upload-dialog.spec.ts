...
vi.mock("@control/asset/asset-data-service", async () => {
...
vi.mock("@control/prize/prize-service", async () => {
...
vi.mock("@control/member/member-service", async () => {
...
import type { PrizeRepository } from "@model/prize/prize-repository";
import type { MemberRepository } from "@model/member/member-repository";
...
const dummyPrizeRepo: Pick<
  ...
  | "deletePrizes"
  | "replaceAllPrizes"
  | "exportAllPrizesToDrive"
  | "importAllPrizesFromDrive"
> = {
  async getPrizes() {
    return [] as any;
  },
...
describe("DataUploadDialog", () => {
  ...
  beforeEach(async () => {
    ...
    const prizeModule = await import("@control/prize/prize-service");
    prizeSvc = new prizeModule.PrizeService(dummyPrizeRepo as any);
    // register into container for DI usage in component
    const assetModule = await import(
      "@control/asset/asset-data-service"
    );
    ...
    const memberModule = await import(
      "@control/member/member-service"
    );
    const memberSvc = new memberModule.MemberService(dummyMemberRepo as any);
