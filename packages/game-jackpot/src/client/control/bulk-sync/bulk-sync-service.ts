...
export class BulkSyncService {
  ...
  private memberRepo = container.resolve(MemberRepository);
  private prizeRepo = container.resolve(PrizeRepository);
  ...
  async syncDirection(
    ...
    try {
      const screenRepo = container.resolve(
        (await import("../../model/screen-config/screen-config-repository"))
