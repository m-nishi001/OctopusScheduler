...
describe("getJsonBlob", () => {
  it("returns an empty array (still success) when the folder property is not configured", async () => {
  ...
  it("reads the default quizzes.json file when no fileId is given", async () => {
    const storage = new InMemoryKeyValueStorage();
  ...
  it("returns an empty array (still success) when the default file is missing", async () => {
    const storage = new InMemoryKeyValueStorage();
    await storage.set(JSON_FOLDER_PROPERTY, "folder-1");
  ...
  it("falls back to a filename-prefix match when the given fileId is not a real key", async () => {
    const storage = new InMemoryKeyValueStorage();
    await storage.set(JSON_FOLDER_PROPERTY, "folder-1");
