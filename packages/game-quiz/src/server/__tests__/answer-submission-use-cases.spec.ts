...
async function setupParticipant(storage: InMemoryKeyValueStorage, userId: string, displayName: string) {
  await addMember({ storage }, { userId, displayName });
  const token = `tok-${userId}-${participantSeq++}`;
  await loginParticipant({ storage, generateToken: () => token }, { userId });
  return token;
}
...
describe("answer-submission-use-cases", () => {
  it("returns an empty list when no answers have been submitted", async () => {
    ...
    expect(await getAnswers({ storage, now: stubClock([]) }, { quizId: "q1" })).toEqual([]);
  });
  ...
  it("records a submitted answer with the server-side timestamp", async () => {
    ...
    const answer = await submitAnswer({ storage, now }, { quizId: "q1", token, optionNo: 2 });
    ...
    expect(await getAnswers({ storage, now }, { quizId: "q1" })).toEqual([answer]);
  });
  ...
  it("ignores a second submission from the same participant and keeps the first", async () => {
    ...
    const first = await submitAnswer({ storage, now }, { quizId: "q1", token, optionNo: 1 });
    const second = await submitAnswer({ storage, now }, { quizId: "q1", token, optionNo: 3 });

    expect(second).toEqual(first);
    expect(await getAnswers({ storage, now }, { quizId: "q1" })).toEqual([first]);
  });
  ...
  it("keeps answers to different quizzes independent", async () => {
    ...
    await submitAnswer({ storage, now }, { quizId: "q1", token, optionNo: 1 });
    await submitAnswer({ storage, now }, { quizId: "q2", token, optionNo: 2 });

    expect(await getAnswers({ storage, now }, { quizId: "q1" })).toHaveLength(1);
    expect(await getAnswers({ storage, now }, { quizId: "q2" })).toHaveLength(1);
  });
  ...
  it("records answers from multiple participants for the same quiz", async () => {
    ...
    await submitAnswer({ storage, now }, { quizId: "q1", token: tokenA, optionNo: 1 });
    await submitAnswer({ storage, now }, { quizId: "q1", token: tokenB, optionNo: 2 });

    const answers = await getAnswers({ storage, now }, { quizId: "q1" });
