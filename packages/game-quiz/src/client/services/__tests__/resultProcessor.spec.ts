...
describe("resultProcessor.computeTopResponders", () => {
  ...
  it("normalizes answers before comparing (trim / numbering / NFKC / case)", () => {
    const answers: AnswerData[] = [
      makeRow(2, "a@example.com", 2000, "回答", "あいうえお", "A"),
      makeRow(3, "b@example.com", 1000, "回答", "1: あいうえお", "B"),
      makeRow(4, "c@example.com", 3000, "回答", " あいうえお ", "C"),
    ];
    ...
    // timestamps: B(1000) then A(2000) then C(3000)
    expect(result[0].name).toBe("B");
    expect(result[1].name).toBe("A");
