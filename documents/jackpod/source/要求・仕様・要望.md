...
container.set(
  "LotteryService",
  new LotteryService(container.get("PrizeRepository"))
);
```
...
- アセットID命名規則：asset*{type}*{timestamp}\_{random}
...
- `DrawService`：抽選サービス（draw-service.ts）
  - インターフェース:
    - `DrawInput`: { members: { id: string; weight: number }[]; prizes: { id: string; weight: number }[]; variable: number; }
    - `DrawPair`: { memberId: string; prizeId: string; }
  - メソッド:
    - `draw(input: DrawInput): DrawPair[]`: メンバーと景品のペアを抽選。重み付きシャッフルと確変調整を実施。
      - ロジック詳細:
        - 景品重みを確変変数で調整（adjustPrizeWeight）。
        - メンバーを重み付きシャッフル（shuffleWithWeights）。
        - 景品を重み付きシャッフル。
        - 最小数（メンバー数または景品数）でペアリング。
    - `adjustPrizeWeight(weight: number, variable: number): number`: 確変変数に基づき景品重みを調整（1: 通常（weightそのまま）, 2: 逆転（11 - weight）, 3: 強い逆転（21 - 2\*weight））。
    - `shuffleWithWeights<T extends { weight: number }>(items: T[]): T[]`: 重み付きシャッフル。重み付き選択で順序をランダム化。
    - `selectWeighted<T extends { weight: number }>(pool: T[]): T`: 重み付き選択。総重みに対するランダム値で項目を選択。
- `Prize`：景品モデル
  - `isHighRank()`
- `Member`：メンバーモデル
  - `isEligible()`
